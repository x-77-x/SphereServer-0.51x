...
const WORD CCompressTree::xCompress_Base[256+1] =	// static
{
...
bool CCompressTree::AddBranch( int Value, WORD wCode, int iBits )
{
    ...
    CCompressTree *pCur=this; 

    if ( ! iBits ) return false;
	...
	for ( ;wMask; wMask >>= 1 )
    {
		...
		{	// if the bit is a 0
			...
			{
				pCur->m_pZero = new CCompressTree;
			}
...
int CCompressTree::Decode( BYTE * pOutput, const BYTE * pInput, int inpsize ) const
{
	...
	int dlen=0;
	const CCompressTree *pCur = this;

	for ( int sbyte=0; sbyte<inpsize; sbyte++ )
	{
		BYTE ch = pInput[sbyte];
		BYTE Mask = 0x80;
		for ( int i=0;i<8;i++)
		{
			if ( ch & Mask )
...
int CCompressTree::Encode( BYTE * pOutput, const BYTE * pInput, int inplen ) // static
{
	...
	{
		WORD value = CCompressTree::xCompress_Base[ ( i == inplen ) ? 256 : pInput[i] ];
		int nBits = value & 0xF;
		value >>= 4;
		while ( nBits-- )
		{
...
void CCryptBase::Init( const BYTE * pSeed )
{
...
int CCryptBase::WriteClientVersion( TCHAR * pszVersion )
{
	int iClientVersion = GetClientVersion();
	return sprintf( pszVersion, _TEXT( "%d.%02d.%02d" ), 
		iClientVersion/10000, 
		(iClientVersion/100)%100, 
		iClientVersion%100 );
}
...
void CCryptBase::Decrypt( BYTE * pOutput, const BYTE * pInput, int iLen )
{
	...
	else if ( GetClientVersion() == 12536 )
	{
		for ( int i=0; i<iLen; i++ )
		{
			pOutput[i] = pInput[i] ^ (BYTE) m_CryptMaskLo;
	...
	else if ( GetClientVersion() ) // GRAY_CLIENT_VER <= 12535
	{
		for ( int i=0; i<iLen; i++ )
		{
			pOutput[i] = pInput[i] ^ (BYTE) m_CryptMaskLo;
...
unsigned char CCrypt::key_table[CRYPT_GAMEKEY_COUNT][6] =
{
...
unsigned char CCrypt::seed_table[2][CRYPT_GAMESEED_COUNT][2][CRYPT_GAMESEED_LENGTH] =
{
...
unsigned int CCrypt::p_table[CRYPT_GAMEKEY_COUNT][18];

unsigned int CCrypt::s_table[CRYPT_GAMEKEY_COUNT][1024];
...
void CCrypt::InitTables()
{
	...
	{
		memcpy(p_table[tempKey], sm_p_box, sizeof(sm_p_box));
		memcpy(s_table[tempKey], sm_s_box, sizeof(sm_s_box));
...
/*
decrypt:
			bIn = m_outbound.decrypt.bufIn.Remove();
			bOut = bIn ^ m_outbound.decrypt.keys.bKeys[m_outbound.decrypt.keys.iIndex];

			m_outbound.decrypt.bufOut.Add(bOut);

			m_outbound.decrypt.keys.bKeys[m_outbound.decrypt.keys.iIndex] = bIn;

			m_outbound.decrypt.keys.iIndex++;
			m_outbound.decrypt.keys.iIndex &= 7;

encrypt:
			bIn = m_outbound.encrypt.bufIn.Remove();
			bOut = bIn ^ m_outbound.encrypt.keys.bKeys[m_outbound.encrypt.keys.iIndex];

			m_outbound.encrypt.bufOut.Add(bOut);

			m_outbound.encrypt.keys.bKeys[m_outbound.encrypt.keys.iIndex] = bOut;

			m_outbound.encrypt.keys.iIndex++;
			m_outbound.encrypt.keys.iIndex &= 7;
*/
