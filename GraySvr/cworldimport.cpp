...
public:
	CImportFile( WORD wModeFlags, CPointMap pt, int iDist ) :
	...
	if ( m_pCurSer != NULL )
	{
		if ( m_pCurObj != NULL && m_pCurSer->m_pObj == m_pCurObj )
		{
			...
			if ( m_pCurSer->IsTopLevel() &&	
				m_p.GetDist( m_pCurObj->GetTopPoint()) > m_iDist )
			{
				delete m_pCurSer;
			}
			else
...
#if 1
				pItemCheck->m_Attr |= ATTR_MOVE_NEVER;
#endif
...
bool CImportFile::ImportSCP( CScript & s, WORD wModeFlags )
{
	...
	{
		CheckLast();
		if ( s.IsSectionType( "WORLDCHAR" ))
		{
			ImportFix();
			if ( wModeFlags & 2 )
			{
				m_pCurObj = CChar::CreateBasic( (CREID_TYPE) s.GetArgHex());
			}
		}
		else if ( s.IsSectionType( "WORLDITEM" ))
		{
			if ( wModeFlags & 1 )
			{
				m_pCurObj = CItem::CreateTemplate( (ITEMID_TYPE) s.GetArgHex());
			}
		}
		else
		{
		...
		while ( s.ReadKeyParse())
		{
			...
			if ( s.IsKey( "CONT" ))
			{
				m_pCurSer->m_dwContSer = s.GetArgHex();
			}
...
bool CImportFile::ImportWSC( CScript & s, WORD wModeFlags )
{
	...
	while ( s.ReadTextLine(true))
	{
		...
		if ( mode == 2 )
		{
			...
			{
				pChar->SetColor( atoi(pArg));
				continue;
			}
			else if ( s.IsKey("DIR" ))
			{
				pChar->m_face_dir = (DIR_TYPE) atoi(pArg);
				continue;
			}
			...
			{
				pChar->m_StatHealth = atoi(pArg);
			}
			else if ( s.IsKey("STAMINA" ))
			{
				pChar->m_StatStam = atoi(pArg);
			}
			else if ( s.IsKey( "MANA" ))
			{
				pChar->m_StatMana = atoi(pArg);
			}
			...
			else if ( s.IsKey("NPCAITYPE" ))
			{
				...
				default:	pChar->SetNPCBrain( pChar->GetCreatureType()); break;
				}
			}
#ifdef COMMENT
				"NPC"
...
bool CWorld::Import( const TCHAR * pszFilename, const CChar * pSrc, WORD wModeFlags, int iDist, TCHAR * pszArg1, TCHAR * pszArg2 )
{
	// wModeFlags = 
	//	 0x01 = items,
	//   0x02 = characters
	//   0x03 = both
	//	 
	//	 0x10 = distance relative. dx, dy to you
	//	 0x20 = recover just this account/char	(and all it is carrying)
	//
	...
	// NOTE: We need to set the World.IsLoading() for this ???
	...
	CImportFile fImport( wModeFlags, pSrc->GetTopPoint(), iDist );

	fImport.m_pszArg1 = pszArg1;
	fImport.m_pszArg2 = pszArg2;

	if ( ! strcmpi( pszFilename + iLen - 4, GRAY_SCRIPT ))
	{
...
bool CWorld::Export( const TCHAR * pszFilename, const CChar * pSrc, WORD wModeFlags, int iDist, int dx, int dy )
{
	...
	if ( wModeFlags & 2 )
	{
		...
		AreaChars.SetInertView( pSrc->IsPriv( PRIV_ALLSHOW ));	// show logged out chars?
		while(true)
		{
